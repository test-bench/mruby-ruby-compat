...
void
mrb_ruby_compat_require_init(mrb_state* mrb) {
  mrb_value require_search_paths;
  mrb_value required_files;
  mrb_value required_files_hash;
...
void
mrb_ruby_compat_require_final(mrb_state* mrb) {
  mrb_int required_files_count;

  required_files_count = mrb_hash_size(mrb, MRUBY_REQUIRED_FILES(mrb));

  debug_printf("Done requiring (Required Files: %d, Load Stack Depth: %d)\n", required_files_count, load_stack_depth());
}
