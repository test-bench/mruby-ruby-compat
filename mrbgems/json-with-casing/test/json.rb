...
assert('symbolize keys') do
  json = '{"someKey": "some-value", "otherKey": { "yetAnotherKey": "other-value" }, "someACRONYMKey": "yet-another-value", "a": "val", "B": "val"}'

  assert_equal({
    :some_key => 'some-value',
    :other_key => {
      :yet_another_key => 'other-value'
    },
    :some_acronym_key => 'yet-another-value',
    :a => 'val',
    :b => 'val'
  }, JSON.parse(json, symbolize_names: true))
end

assert('symbol keys to camel cased JSON strings') do
  assert_equal('{"someKey":"some-value"}', JSON.generate({ :some_key => 'some-value' }))
  assert_equal('{"otherKey":"some-value"}', JSON.generate({ :other_key => 'some-value' }))
  assert_equal('{"yetAnotherKey":"some-value"}', JSON.generate({ :yet_another_key => 'some-value' }))
  assert_equal('{"someAcronymKey":"some-value"}', JSON.generate({ :some_acronym_key => 'some-value' }))
  assert_equal('{"_someKey_":"some-value"}', JSON.generate({ :_some_key_ => 'some-value' }))
  assert_equal('{"a":"some-value"}', JSON.generate({ :a => 'some-value' }))
  assert_equal('{"b_":"some-value"}', JSON.generate({ :b_ => 'some-value' }))
end
