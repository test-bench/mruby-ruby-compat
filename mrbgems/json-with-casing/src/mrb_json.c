...
mrb_value_to_string(mrb_state* mrb, mrb_value value, int pretty) {
  ...
  case MRB_TT_UNDEF:
    ...
    break;
  case MRB_TT_SYMBOL:
    str = ruby_symbol_to_json_key(mrb, mrb_symbol(value));
    break;
    /* FALLTHROUGH */
  case MRB_TT_STRING:
    {
  ...
  case MRB_TT_HASH:
    {
      ...
      for (n = 0; n < l; n++) {
        ...
        if(mrb_type(key) == MRB_TT_SYMBOL) {
          enckey = ruby_symbol_to_json_key(mrb, mrb_symbol(key));
        } else {
          enckey = mrb_funcall(mrb, key, "to_s", 0, NULL);
...
static mrb_value
json_value_to_mrb_value(mrb_state* mrb, JSON_Value* value, mrb_bool symbolize_names) {
  mrb_value ret;
  ...
  case JSONObject:
    {
      ...
      for (n = 0; n < count; n++) {
        ...
        if(symbolize_names) {
          key = json_key_to_ruby_symbol(mrb, name);
        } else {
          key = mrb_str_new_cstr(mrb, name);
        }
...
mrb_json_load(mrb_state *mrb, mrb_value self)
{
  ...
  value = json_value_to_mrb_value(mrb, root_value, FALSE);
...
mrb_json_parse(mrb_state *mrb, mrb_value self)
{
  ...
  static const char* symbolize_names_str = "symbolize_names";

  uint32_t kw_num = 1;
  const char* kw_names[] = { symbolize_names_str };
  ...
  symbolize_names = mrb_bool(kw_values[0]);
...
void
mrb_mruby_json_with_casing_gem_init(mrb_state* mrb) {
  ...
  mrb_define_class_method(mrb, _class_json, "parse", mrb_json_parse, MRB_ARGS_REQ(1) | MRB_ARGS_KEY(1, 0));
...
void
mrb_mruby_json_with_casing_gem_final(mrb_state* mrb) {
}
